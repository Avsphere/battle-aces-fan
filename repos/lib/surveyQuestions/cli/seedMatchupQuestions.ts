...
console.log(chalk.blueBright(`Reading matchup questions from ${getPathOfManuallyCreatedMatchupQuestions()}`))

const manuallySuppliedQuestions = await readCurrentManuallyCreatedMatchupQuestions()

console.log(chalk.blueBright(`Read ${manuallySuppliedQuestions.length} questions`))

const allExistingQuestions = await repos.surveyQuestions.questions.findAll()

// now filter out questions that are the same
const newQuestions = manuallySuppliedQuestions.filter((manuallySuppliedQuestion) => {

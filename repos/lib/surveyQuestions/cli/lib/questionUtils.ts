...
import z from "zod";
import { SurveyQuestion_BasicSchema, SurveyQuestion_UnitMatchupSchema, SurveyQuestionDetails, Unit } from "@battle-aces-fan/datacontracts";
...
export const tryFindUnitBySlugSubString = (units : Unit[], slugSubString: string) => {
...
export const readCurrentManuallyCreatedMatchupQuestions = async() => {
  const pathToRead = getPathOfManuallyCreatedMatchupQuestions();
  const jsonStr = await Deno.readTextFile(pathToRead);
  const parsed = z.array(SurveyQuestion_UnitMatchupSchema).parse(JSON.parse(jsonStr));

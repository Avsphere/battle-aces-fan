...
import { SurveyQuestion_UnitMatchupSchema, SurveyQuestionDetails } from "@battle-aces-fan/datacontracts";
...
export const BASIC_UNIT_QUESTION_TAGS = [
    'glitchy movement', 'fun to play against', 'annoying to play against', 'overpowered', 'underpowered', 'feels great to control',
    'looks bad', 'useless', 'overused', 'low skill expression', 'high skill expression'
] as const

export const BASIC_UNIT_MATCHUP_TAGS = [
    'fun',
    'brutally snowballs', 'boring', 'frustrating', 'low skill expression', 'high skill expression',
] as const
...
const generateBasicUnitMatchupQuestions = () : SurveyQuestionDetails[] => {
    ...
    const baseTierUnitPairsIncludingSelf = baseTierUnits.flatMap((unit1, index) => {
        return baseTierUnits.slice(index).map((unit2) => {
            return [unit1, unit2]
        })
    })

    const pairsWhereBothUnitsOnlyTargetGround = baseTierUnitPairsIncludingSelf.filter(([unit1, unit2]) => {
        return unit1.details.targetsAir === false && unit2.details.targetsAir === false
    })

    const questions = pairsWhereBothUnitsOnlyTargetGround.map(([unit1, unit2]) => {
        const question : SurveyQuestionDetails = {
            kind : 'basic',

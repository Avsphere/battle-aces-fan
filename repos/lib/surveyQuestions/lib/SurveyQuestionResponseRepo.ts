...
export const SurveyQuestionResponseRepo = (repos : Repos) => {
    ...
    const findByQuestionKind = async(params: Parameters<SurveyQuestionResponseModel['findByQuestionKind']>[0]) => {
        return await appContext.models.surveyQuestionResponses.findByQuestionKind(params)
    }

    const findByUser = async(params: Parameters<SurveyQuestionResponseModel['findAllByUser']>[0]) => {
        return await appContext.models.surveyQuestionResponses.findAllByUser(params)
    }

    const create = async(params: Parameters<SurveyQuestionResponseModel['create']>[0]) => {

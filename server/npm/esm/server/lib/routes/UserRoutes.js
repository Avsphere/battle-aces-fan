...
export const UserRoutes = (repos) => {
    const userRoutes = new Hono()
        .post("/find-or-create", zValidator("json", GetOrCreateUserSchema), async (c) => {
        ...
        .get("/questions/:userId", zValidator("param", z.object({ userId: z.string() })), async (c) => {
        const userId = c.req.param().userId;
        const questions = await repos.users.findAllUnansweredQuestions(userId);
        return c.json({ questions: questions.map(q => q.data) });
    })
        .post("/responses/:userId", zValidator("json", UserSubmitResponseSchema), async (c) => {
        const userId = c.req.param().userId;

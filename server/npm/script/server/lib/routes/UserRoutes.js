...
const UserRoutes = (repos) => {
  const userRoutes = new hono_1.Hono()
    .post(
      "/find-or-create",
      (0, zod_validator_1.zValidator)("json", GetOrCreateUserSchema),
      async (c) => {
        ...
        const user = await app_context_1.appContext.models.users.findById(
          data.userId,
        );
        return c.json({
          user: user.data,
    ...
    .get(
      "/questions/:userId",
      (0, zod_validator_1.zValidator)(
        "param",
        zod_1.z.object({ userId: zod_1.z.string() }),
      ),
      async (c) => {
    ...
    .post(
      "/responses/:userId",
      (0, zod_validator_1.zValidator)("json", UserSubmitResponseSchema),
      async (c) => {
        const userId = c.req.param().userId;

...
import {
  ...
  type SurveyQuestionResponseSchema,
...
export class SurveyQuestionResponseModel {
  ...
  findAllByUser = async (userId: string) => {
    const key =
      "details.userId" as unknown as keyof SurveyQuestionResponseSchema;
  ...
  findByQuestionKind = async (kind: string) => {
    const key = "details.kind" as unknown as keyof SurveyQuestionResponseSchema;
  ...
  findByQuestionId = async (questionId: string) => {
    const key =
      "details.questionId" as unknown as keyof SurveyQuestionResponseSchema;

...
export class UnitModel {
  ...
  static create = (db: Db): UnitModel => new UnitModel(UnitCollection(db));
  ...
  findBySlug = async (slug: string): Promise<Unit[]> => {
    const key = "details.slug" as unknown as keyof UnitSchema;
  ...
  findByTechTier = async (techTierSlug: string): Promise<Unit[]> => {
    const key = "details.techTier.slug" as unknown as keyof UnitSchema;
    const docs = await this.collection.find({ [key]: techTierSlug }).toArray();
    return docs.map(this.toDocument);
  };

  findByName = async (name: string): Promise<Unit[]> => {
    const key = "details.name" as unknown as keyof UnitSchema;
  ...
  findByManufacturer = async (manufacturerSlug: string): Promise<Unit[]> => {
    const key = "details.manufacturer.slug" as unknown as keyof UnitSchema;
    const docs = await this.collection.find({ [key]: manufacturerSlug })
      .toArray();
    return docs.map(this.toDocument);
  };
